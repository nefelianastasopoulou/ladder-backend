...
const startMemoryMonitoring = () => {
  ...
  const MEMORY_THRESHOLD = NODE_ENV === 'production' 
    ? 500 * 1024 * 1024  // 500MB for production
    : 200 * 1024 * 1024; // 200MB for development
  
  const MEMORY_CHECK_INTERVAL = NODE_ENV === 'production' 
    ? 300000  // 5 minutes for production
    : 60000;  // 1 minute for development
  
  let lastMemoryLog = 0;
  const MEMORY_LOG_INTERVAL = 1800000; // 30 minutes
  ...
  const memoryMonitor = setInterval(() => {
    ...
    // Log memory usage periodically in production (not every check)
    if (NODE_ENV === 'production' && (now - lastMemoryLog) > MEMORY_LOG_INTERVAL) {
    ...
    if (memUsage.heapUsed > MEMORY_THRESHOLD) {
      ...
      // In production, suggest garbage collection
  ...
  logger.info(`Memory monitoring started (threshold: ${Math.round(MEMORY_THRESHOLD / 1024 / 1024)}MB, interval: ${MEMORY_CHECK_INTERVAL / 1000}s)`);
};
...
const validateFileUpload = (req, res, next) => {
  ...
  if (req.file.size < 100) {
    return sendErrorResponse(res, 400, 'File too small. Please upload a valid image.');
  }
  ...
  if (suspiciousPatterns.test(req.file.originalname)) {
    return sendErrorResponse(res, 400, 'Invalid filename. Please use a valid filename.');
  }
  
  // Check filename length
  if (req.file.originalname.length > 255) {
    return sendErrorResponse(res, 400, 'Filename too long. Maximum 255 characters allowed.');
  }
  ...
  try {
    ...
    // Additional security: Check for embedded scripts or executable content
    const suspiciousContent = /<script|javascript:|vbscript:|onload=|onerror=|eval\(|document\.|window\./i;
    const fileContent = fileBuffer.toString('utf8', 0, Math.min(fileBuffer.length, 1024));
    if (suspiciousContent.test(fileContent)) {
      fs.unlinkSync(req.file.path);
      return sendErrorResponse(res, 400, 'File contains suspicious content.');
    }
    
    // Check for potential zip bombs or compressed files
    if (fileBuffer.length > 0 && (fileBuffer[0] === 0x50 && fileBuffer[1] === 0x4B)) { // ZIP signature
      fs.unlinkSync(req.file.path);
      return sendErrorResponse(res, 400, 'Compressed files are not allowed.');
    }
...
// Run environment validation (non-blocking)
try {
  validateEnvironmentVariables();
} catch (error) {
  logger.warn('Environment validation error:', error.message);
...
const authLimiter = rateLimit({
  ...
  max: 10, // Increased from 5 to 10 for better user experience
  message: {
    success: false,
    error: {
...
const signupLimiter = rateLimit({
  ...
  max: 5, // Increased from 3 to 5 for better user experience
  message: {
    success: false,
    error: {
...
const uploadLimiter = rateLimit({
  windowMs: 15 * 60 * 1000, // 15 minutes
  max: 20, // Increased from 10 to 20 for better user experience
  message: {
    success: false,
    error: {
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // Development origins (only when NODE_ENV is development)
    ...
    // Allow requests with no origin only in development (for mobile apps)
    if (!origin && NODE_ENV === 'development') {
      return callback(null, true);
    }
    
    // In development, allow localhost with any port and ngrok
    if (NODE_ENV === 'development') {
      if (origin.match(/^http:\/\/localhost:\d+$/) || 
          origin.match(/^http:\/\/127\.0\.0\.1:\d+$/) ||
          origin.match(/^exp:\/\/.*$/) ||
          origin.match(/^http:\/\/.*\.ngrok\.io$/)) {
        return callback(null, true);
      }
    }
...
// Serve uploaded files statically with security headers (for local storage)
...
// Endpoint to serve images with signed URLs (for S3)
app.get('/api/images/:fileKey(*)', authenticateToken, async (req, res) => {
  try {
    const { fileKey } = req.params;
    
    // Generate signed URL for the file
    const signedUrl = await cloudStorage.getSignedUrl(fileKey, 3600); // 1 hour expiry
...
const initializeApp = async () => {
  try {
    ...
    try {
      ...
      logger.warn('Database migration failed:', error.message);
      // Continue anyway
    }
    ...
    try {
      ...
      if (dbConnected) {
        ...
        logger.warn('⚠️ Database connection failed - will retry later');
      }
    } catch (error) {
...
const cleanupOrphanedFiles = async () => {
  try {
    ...
    for (const file of files) {
      ...
      // Check if file is older than 24 hours and not referenced in database
      const fileAge = Date.now() - stat.mtime.getTime();
      const maxAge = 24 * 60 * 60 * 1000; // 24 hours
      
      if (fileAge > maxAge) {
        // Check if file is referenced in database
        const isReferenced = await new Promise((resolve) => {
          db.query(
            'SELECT id FROM posts WHERE image_url LIKE $1',
            [`%${file}%`],
            (err, result) => {
              if (err) {
                logger.error('Error checking file reference:', err);
                resolve(false);
              } else {
        ...
        if (!isReferenced) {
          try {
            ...
            logger.error(`Error deleting file ${file}:`, unlinkError);
          }
        }
      }
    }
    ...
    if (cleanedCount > 0) {
      logger.info(`File cleanup completed: ${cleanedCount} orphaned files removed`);
    }
  } catch (error) {
...
// Detailed health check endpoint - tests database connection with timeout
app.get('/health/detailed', async (req, res) => {
  try {
    // Test database connection with timeout
    const dbStartTime = Date.now();
    const dbTimeout = 5000; // 5 second timeout for health check
    
    const dbTestPromise = new Promise((resolve, reject) => {
...
app.get('/api/docs', (req, res) => {
  sendSuccessResponse(res, 200, apiDocumentation);
});
...
app.get('/api/search/posts', authenticateToken, readOnlyLimiter, (req, res) => {
  ...
  db.query(
    `SELECT p.id, p.title, p.content, p.image_url, p.image_key, p.category, p.likes_count, p.comments_count, p.created_at,
    ...
    async (err, result) => {
      ...
      // Convert S3 URLs to signed URLs for private bucket
      const postsWithSignedUrls = await Promise.all((result.rows || []).map(async (post) => {
        if (post.image_key && process.env.STORAGE_TYPE === 's3') {
          try {
            const signedUrl = await cloudStorage.getSignedUrl(post.image_key, 3600);
      ...
      sendSuccessResponse(res, 200, postsWithSignedUrls);
    }
  );
});
...
app.get('/api/search/communities', authenticateToken, readOnlyLimiter, (req, res) => {
  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        logger.error('Error searching communities:', err);
        return res.status(500).json({ error: 'Database error' });
      }
      sendSuccessResponse(res, 200, result.rows || []);
    }
...
app.get('/api/search/all', authenticateToken, readOnlyLimiter, (req, res) => {
  ...
  if (!q || q.trim() === '') {
    return sendSuccessResponse(res, 200, { users: [], posts: [], communities: [] });
  }
    ...
    (err, users) => {
      ...
      db.query(
        `SELECT p.id, p.title, p.content, p.image_url, p.image_key, p.category, p.likes_count, p.comments_count, p.created_at,
         ...
         LEFT JOIN users u ON p.author_id = u.id
         LEFT JOIN communities c ON p.community_id = c.id
         WHERE p.is_published = true AND (p.title LIKE $1 OR p.content LIKE $2)
         ORDER BY 
           CASE 
        ...
        async (err, posts) => {
          ...
          db.query(
            `SELECT c.id, c.name, c.description, c.category, c.member_count, c.is_public, c.created_at,
             ...
             FROM communities c
             LEFT JOIN users u ON c.created_by = u.id
             WHERE c.is_public = true AND (c.name LIKE $1 OR c.description LIKE $2)
             ORDER BY 
               CASE 
  ...
  db.query(
    ...
    [user_id],
        function(err, result) {
          ...
          sendSuccessResponse(res, 200, null, 'User promoted to admin successfully');
        }
  );
});
          ...
          (err) => {
            ...
            db.query(
              ...
              (err) => {
                ...
                sendSuccessResponse(res, 200, null, 'Profile updated successfully');
              }
            );
          }
        );
...
app.get('/api/settings', authenticateToken, (req, res) => {
  db.query(
    ...
    (err, settings) => {
      ...
      sendSuccessResponse(res, 200, settings || defaultSettings);
    }
  );
});
...
app.put('/api/settings', authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    function(err, result) {
      ...
      sendSuccessResponse(res, 200, null, 'Settings updated successfully');
    }
  );
});
...
app.get('/api/opportunities/my', authenticateToken, (req, res) => {
  db.query(
    ...
    (err, opportunities) => {
      ...
      sendSuccessResponse(res, 200, opportunities || []);
    }
  );
});
...
app.get('/api/favorites', authenticateToken, (req, res) => {
  db.query(
    ...
    (err, favorites) => {
      ...
      sendSuccessResponse(res, 200, favorites || []);
    }
  );
});
...
app.get('/api/communities', authenticateToken, (req, res) => {
  ...
  db.query(
    ...
    (err, communities) => {
      ...
      sendSuccessResponse(res, 200, communities || []);
    }
  );
});
...
app.get('/api/communities/:id/posts', authenticateToken, (req, res) => {
  ...
  db.query(
    `SELECT p.id, p.title, p.content, p.image_url, p.image_key, p.likes_count, p.comments_count, p.created_at,
            u.full_name as author_name, u.username as author_username
     FROM posts p
     LEFT JOIN users u ON p.author_id = u.id
...
app.post('/api/communities/:id/posts', authenticateToken, uploadLimiter, checkPhotoUploadRestrictions, (req, res, next) => {
...
}, validateImageFile, validateFileUpload, async (req, res) => {
  const communityId = req.params.id;
  ...
  if (req.file) {
    try {
      const uploadResult = await cloudStorage.uploadFile(req.file, 'community-posts');
      imageUrl = uploadResult.url;
      imageKey = uploadResult.key; // Store S3 key for signed URLs
    } catch (error) {
  ...
  db.query(
    ...
    (err, result) => {
      ...
      // Create the post
      db.query(
        'INSERT INTO posts (title, content, author_id, community_id, image_url, image_key, likes_count, comments_count, created_at) VALUES ($1, $2, $3, $4, $5, $6, 0, 0, CURRENT_TIMESTAMP) RETURNING id',
        [title, content, userId, communityId, imageUrl, imageKey],
        function(err, result) {
          if (err) {
...
app.post('/api/posts', authenticateToken, uploadLimiter, checkPhotoUploadRestrictions, (req, res, next) => {
  ...
  if (req.file) {
    try {
      const uploadResult = await cloudStorage.uploadFile(req.file, 'platform-posts');
      imageUrl = uploadResult.url;
      imageKey = uploadResult.key; // Store S3 key for signed URLs
    } catch (error) {
      logger.error('Error uploading file:', error);
  ...
  db.query(
    'INSERT INTO posts (title, content, author_id, community_id, image_url, image_key, likes_count, comments_count, created_at) VALUES ($1, $2, $3, NULL, $4, $5, 0, 0, CURRENT_TIMESTAMP) RETURNING id',
    [title, content, userId, imageUrl, imageKey],
    function(err, result) {
      if (err) {
    ...
    function updateCommunity() {
      ...
      db.query(
        ...
        function(err, result) {
          ...
          sendSuccessResponse(res, 200, null, 'Community updated successfully');
        }
      );
    }
  });
...
app.post('/api/communities', authenticateToken, (req, res) => {
  ...
  db.query('SELECT id FROM communities WHERE name = $1', [name], (err, result) => {
    ...
    db.query(
      ...
      [name, description, category || '', req.user.id, 1, true],
      function(err, result) {
        if (err) {
          return res.status(500).json({ error: 'Database error' });
        }
  ...
  db.query('SELECT * FROM community_members WHERE user_id = $1 AND community_id = $2', [userId, communityId], (err, result) => {
    ...
    db.query(
      ...
      function(err, result) {
        ...
        sendSuccessResponse(res, 200, null, 'Successfully left the community');
      }
    );
  });
});
...
app.get('/api/conversations', authenticateToken, (req, res) => {
  ...
  db.query(query, [userId, userId, userId], (err, rows) => {
    ...
    sendSuccessResponse(res, 200, conversations);
  });
});
  ...
  db.query('SELECT is_admin FROM users WHERE id = $1', [userId], (err, result) => {
    ...
    // Delete messages in community posts
      ...
      db.query('DELETE FROM communities WHERE id = $1', [communityId], function(err, result) {
        ...
        sendSuccessResponse(res, 200, null, 'Community deleted successfully');
      });
    });
  });
...
app.put('/api/admin/reports/:id', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.query(
    ...
    function(err, result) {
      ...
      sendSuccessResponse(res, 200, null, 'Report status updated successfully');
    }
  );
});
...
app.get('/api/admin/posts', authenticateToken, (req, res) => {
  ...
  db.query('SELECT is_admin FROM users WHERE id = $1', [userId], (err, result) => {
    ...
    db.query(query, async (err, posts) => {
      if (err) {
...
// Graceful shutdown is now handled above
